...
namespace Atemoya.Classes.Github {
        ...
        public static async Task<UpdateVersion> GetUpdates() {
            try {
                using (var client = new WebClient()) {
                    client.Headers.Add(HttpRequestHeader.UserAgent, $"ATEMOYA/{Assembly.GetExecutingAssembly().GetName().Version} (release; PC)");
                    using (var stream = new MemoryStream(await client.DownloadDataTaskAsync(new Uri("https://api.github.com/repos/Twigzie/Fantality-Halo-Atemoya/releases/latest")))) {
